...
%% v5 — Updated latency numbers (Redis backend, 2026-04-01 run), CI95 column,
%%       fixed em-dashes, ablation methodology note, Practical Adoption Checklist
%%
...
baseline. The pattern is authorization-server-agnostic and requires no changes
to the agent layer.
\end{abstract}
...
\begin{enumerate}
  \item \textbf{Verify IdP support}: Confirm your identity provider supports the
        RFC 8693 \texttt{urn:ietf:params:oauth:grant-type:token-exchange} grant
        (Keycloak $\geq$17, Microsoft Entra OBO, Auth0 Token Vault, ZITADEL,
        Authlete all qualify). If not, deploy a lightweight exchange sidecar.
