...
30 trials. The ablation study confirms that each of the three security properties
is independently necessary---no partial combination blocks all four attacks.
Per-hop cryptographic overhead measures $\approx 2.15$\,ms (one RSA-2048 signing
operation plus one revocation store update), yielding $O(k)$ total overhead for
a $k$-hop delegation chain, with $R^2 = 0.9999$ linearity.
...
valid only for that service. Our tests show this blocks all four known attacks
with no false positives, adds roughly 2\,ms per service call in raw cryptographic
cost, and leaves a complete, verifiable record of every delegation step.
\end{IEEEkeywords}
