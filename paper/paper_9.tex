...
team adopting the NAC pattern in an existing MCP deployment. First,
\textbf{verify IdP support}: confirm your identity provider supports the RFC 8693
\texttt{urn:ietf:params:oauth:grant-type:token-exchange} grant type (Keycloak
$\geq$17, Microsoft Entra OBO, Auth0 Token Vault, ZITADEL, and Authlete all
qualify); if not, deploy a lightweight exchange sidecar as described in
Section~\ref{sec:auth0_validation}. Second, \textbf{register per-service
audiences}: each worker service must have a registered OAuth resource identifier,
which is the only configuration change required at the identity provider. Third,
\textbf{update the hub}: replace the single forwarded token with one
\texttt{POST /token/exchange} call per worker, fired concurrently using
\texttt{asyncio.gather} or an equivalent parallel construct---the total code
change is fewer than 50 lines. Fourth, \textbf{enable worker validation}: each
worker must verify the \texttt{aud}, \texttt{scope}, \texttt{act} chain depth,
and \texttt{jti} (revoke after first use) via a shared middleware layer to
...
\textit{Concurrent agent stress test}: running multiple simultaneous agents that
share a root token would verify that the revocation store handles concurrent
