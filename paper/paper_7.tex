...
study across six enforcement configurations. The sidecar integration path is
validated end-to-end against a live Auth0 tenant ($N=30$).
...
\textbf{Conclusions:}
The RFC~8693 NAC pattern provides cryptographically grounded, measurable, and
deployable security for MCP agentic workflows by construction. Production
deployment overhead is projected at $+7$--$15\%$ relative to an insecure
baseline. The pattern is authorization-server-agnostic---confirmed end-to-end with a live
Auth0 tenant and requires no changes to the agent layer.
\end{abstract}
...
This section establishes the technical context for the paper. We introduce the
LLM agent model underlying MCP tool use, summarise the Model Context Protocol,
review the relevant OAuth and JWT standards (including RFC 8693 token exchange
and RFC 7009 revocation), and map the problem to the OWASP Generative AI threat
taxonomy.
...
is enforced at workers. The implementation is intentionally minimal each
component is a single Python file using FastAPI and standard libraries so that
the security-relevant logic is immediately auditable without framework noise.
...
The measured overhead of $+35.6\%$ ($125.1 \pm 3.7$\,ms baseline vs.\
$169.6 \pm 4.7$\,ms secure; 95\% CI computed via $t_{29} = 2.045$) reflects
a single-machine, single-worker authorization server. The non-overlapping CI
intervals confirm statistical significance. The $\sigma$-ratio of $1.26\times$
...
signing operation plus one revocation store write). This linearity follows from
first principles: each hop performs an independent constant-cost operation with no
...
extension. For organizations using an IdP without native RFC 8693 support, a
lightweight token exchange sidecar can be deployed to proxy exchange requests
to the existing IdP; this path was validated end-to-end against Auth0 in
Section~\ref{sec:auth0_validation}.

\subsection{Practical Adoption Checklist}

The following four steps cover the minimum integration surface for an engineering
team adopting the NAC pattern in an existing MCP deployment:

\begin{enumerate}
  \item \textbf{Verify IdP support}: Confirm your identity provider supports the
      RFC~8693
      \texttt{urn:ietf:\allowbreak params:\allowbreak oauth:\allowbreak
      grant-type:\allowbreak token-exchange}
      grant (Keycloak~$\geq$17, Microsoft Entra OBO, Auth0 Token Vault,
      ZITADEL, Authlete all qualify). If not, deploy a lightweight exchange
      sidecar.
  \item \textbf{Register per-service audiences}: Each worker service must have a
        registered OAuth resource identifier. This is the only configuration change
        required at the identity provider.
  \item \textbf{Update the hub}: Replace the single forwarded token with one
        \texttt{POST /token/exchange} call per worker, fired concurrently using
        \texttt{asyncio.gather} or equivalent. Total code change: $<50$ lines.
  \item \textbf{Enable worker validation}: Each worker must verify \texttt{aud},
        \texttt{scope}, \texttt{act} chain depth, and \texttt{jti} (revoke after
        first use). Use a shared middleware; do not duplicate per-service.
\end{enumerate}
...
\begin{enumerate}
  ...
  \item \textit{Additional IdP portability}: Auth0 M2M validation is complete
        (Section~\ref{sec:auth0_validation}); confirming portability against
        Keycloak 17 and Microsoft Entra OBO remains open.
  \item \textit{Formal verification}: Model the exchange protocol in ProVerif
        ...
        model.
  \item \textit{Persistent-session token caching}: Evaluate whether caching
        recently issued child tokens for a short TTL (e.g., 5\,s) can reduce
        per-request exchange overhead in high-frequency agentic loops.
\end{enumerate}
...
\begin{enumerate}
  ...
  \item Per-hop cryptographic overhead is measured at $\approx 2.15$\,ms
        (one RSA-2048 sign plus one revocation store update), yielding $O(k)$
        total cost for $k$-hop chains ($R^2 = 0.9999$ over $k=1\ldots10$).
        End-to-end measured overhead is $+35.6\%$ on a single machine;
        production deployment overhead is projected at $+7$--$15\%$ relative
        to the insecure baseline with a dedicated authorization server.
  \item The RFC 8693 sidecar integration path was validated end-to-end against
        a live Auth0 tenant ($N=30$). All four attacks were blocked in 100\% of
        trials with real Auth0 M2M root tokens, confirming that the pattern is
        authorization-server-agnostic by construction and requires no changes
        to the worker services.
\end{enumerate}
